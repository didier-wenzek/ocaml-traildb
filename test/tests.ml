...
let check_single_trail _test_ctxt =
    let file = temp_traildb_file () in
    let trail = "UTMB" in
...
let creating_a_known_file _test_ctxt =
  assert_raises TrailDB.(Error TDB_ERR_IO_OPEN)
  (fun () -> create_traildb ~file:(Filename.temp_file "traildb_" "_test") ~fields:["a"] ~events:[])

let opening_an_unknown_file _test_ctxt =
  assert_raises TrailDB.(Error TDB_ERR_IO_OPEN)
  (fun () -> traildb_num_trails ~file:"/foo")
